...
const VerifyElectorScreen = () => {
	...
	const handleAction = {
		...
		onSearch: () => {
			...
			updateParam("findElectorRegister", (isNumeric ? "prkdelektor" : "prnama"), formattedText);
							...
							{isLoading ? (
								...
								<>
									{data.findElectorRegister.rows.map((elector, index) => (
										<TouchableOpacity key={index} onPress={() => router.push({pathname: `/dashboard/electors/${elector?.KODE_ELEKTOR}`, params: {keyParam: "findElectorRegister", historySearchText: searchText, fromScreen: APP_ROUTES.DASHBOARD.VERIFY_ELECTOR}})}>
