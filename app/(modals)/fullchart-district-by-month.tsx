...
export default function FullChartDistrictByMonthScreen() {
	const viewShotRef = useRef(null);

	const {user} = useAuth();
	const {fetchChartByMonth, paramChartbyMonth, isLoadingChartByMonth, setParams: setParamsChart, data: dataChart} = useReport()
	const {screenOrigin} = useLocalSearchParams();
	const [showModalDate, setShowModalDate] = useState(false);

	const generateDataChartbyMonth = dataChart?.reportChartByMonth || []
					...
					<View className="flex justify-center items-center w-full h-full py-10">
						{isLoadingChartByMonth ? (
							...
							<ViewShot ref={viewShotRef} options={{ format: 'jpg', quality: 0.9 }}>
								<ChartDistrictBarByMonth handleAction={handleAction} barData={dataChartByMonth} period={formatDate(parseFormattedDate(paramChartbyMonth?.date), "yyyy")} isFromFullchart={true} />
							</ViewShot>
						)}
					</View>
				</ScrollView>
